...
- The image is **fully generic** — no baked config. SSH key, k3s role/token and the COS_PERSISTENT grow stage all arrive per node via Incus `cloud-init.user-data` (`opentofu/templates/`), so rotating the key or changing config is a `tofu apply` + reboot, not a re-image.
...
- Build: `make image` from `opentofu/` (or `./build.sh`).
