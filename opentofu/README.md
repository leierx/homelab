# opentofu — k3s clusters on Incus
...
- `/dev/kvm` + qemu/xorriso/kubectl/yq from the host config (see `image/README.md`).
...
tofu init
tofu apply
make kubeconfig  # -> ~/.kube/config-hadron
```
...
- One file per cluster at the root: `prod.tf`, `test.tf`, `mgmt.tf` — network, token and all VMs, hardcoded and explicit. Tofu only loads `*.tf` from the root dir, so they can't live in a subdirectory.
  ...
  - add a worker: one line in the cluster's slave `for_each` map
...
- `templates/` — the three cloud-init role templates (controlplane-init/-join, worker): SSH key, partition grow stage, k3s role/flags.
- `main.tf` — storage pool + the one shared Kairos golden image; all per-node config is injected via `cloud-init.user-data` + the `cloud-init:config` disk.
