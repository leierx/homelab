...
services.haproxy = {
  ...
  config = ''
    ...
    frontend fe_tls
      ...
      use_backend be_prod if { req_ssl_sni -m end -i .prod.example.com }
      use_backend be_test if { req_ssl_sni -m end -i .test.example.com }
      default_backend be_drop

    frontend fe_http
      bind :80
      mode http
      use_backend be_prod_http if { hdr(host) -m end -i .prod.example.com }
      use_backend be_test_http if { hdr(host) -m end -i .test.example.com }
